...
impl SimpleExpression {
    fn new(identifier: String, document_ref: Option<String>, license_ref: bool) -> Self {
        Self {
            identifier,
            document_ref,
            license_ref,
...
#[cfg(test)]
mod tests {
    use crate::{document_ref, idstring, license_ref, simple_expression, SimpleExpression};
    ...
    fn simple_expression_with_document_ref() {
        let (_, result) = simple_expression("DocumentRef-SPDX-DOC:LicenseRef-New-license-1.0").unwrap();
        assert_eq!(
            result,
            SimpleExpression {
